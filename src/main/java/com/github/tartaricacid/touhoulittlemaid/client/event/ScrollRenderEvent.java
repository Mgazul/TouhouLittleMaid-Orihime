...
public class ScrollRenderEvent {
    ...
    public static void onRenderWorldLastEvent(WorldRenderContext context) {
        ...
        if (/*RenderLevelStageEvent.Stage.AFTER_PARTICLES.equals(event.getStage()) &&*/ player != null) {
            ...
            RenderHelper.renderFloatingText(context.matrixStack(), Math.round(actualDistance) + " m", trackVec, 0xff8800, scale, -17);
            RenderHelper.renderFloatingText(context.matrixStack(), "▼", trackVec, 0xff0000, scale * 1.2f, -5);
        }
    }
