...
public class MaidEmiPlugin implements EmiPlugin {
    ...
    private void hideStacks(EmiRegistry registry) {
        ...
        groupItems.add(InitItems.GARAGE_KIT.getDefaultInstance());
        groupItems.add(InitItems.CHAIR.getDefaultInstance());
        ...
        for (ItemStack item : groupItems) {
            registry.addEmiStackAfter(EmiStack.of(item), emiStack -> emiStack.getItemStack().is(InitItems.POWER_POINT));
        }
    }
    ...
    private void registerInfoRecipe(EmiRegistry registry) {
        registry.addRecipe(getInfoRecipe(InitItems.GARAGE_KIT, Component.translatable("jei.touhou_little_maid.garage_kit.info")));
    }
