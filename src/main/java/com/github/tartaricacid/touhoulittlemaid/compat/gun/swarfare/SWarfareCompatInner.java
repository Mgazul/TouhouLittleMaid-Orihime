...
public class SWarfareCompatInner {
    ...
    private static void useGrenade(EntityMaid shooter, LivingEntity target) {
        ...
        if (offhand.getItem() instanceof RgoGrenade && shooter.distanceTo(target) <= 16) {
            ...
            shooter.level.playSound(null, shooter.blockPosition(), ModSounds.GRENADE_THROW, SoundSource.NEUTRAL, 1, 1);
            offhand.shrink(1);
        }
    }
    ...
    private static int doGunReload(EntityMaid shooter, GunData gunData) {
        if (gunData.shouldStartReloading(shooter)) {
            gunData.startReload();
            NetworkHandler.sendToTrackingEntity(shooter, MaidAnimationMessage.ID, MaidAnimationMessage.encode(shooter.getId(), SWF_RELOAD));
            return 5;
        }
    ...
    private static int doGunShoot(EntityMaid shooter, LivingEntity target, ItemStack gunItem, GunData gunData) {
        ...
        // 播放开火动作
        NetworkHandler.sendToTrackingEntity(shooter, MaidAnimationMessage.ID, MaidAnimationMessage.encode(shooter.getId(), SWF_FIRE));
        return cooldown;
    }
