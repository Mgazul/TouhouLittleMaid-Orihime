...
import cn.sh1rocu.touhoulittlemaid.mixin.accessor.MinecraftAccessor;
...
public abstract class AnimatableEntity<E extends Entity> {
    ...
    public double getCurrentTick(AnimationEvent<?> animationEvent) {
        ...
        float realPartialTick = client.isPaused() ? ((MinecraftAccessor) client).tlm$getPausePartialTick() : client.getFrameTime();
        if (partialTick == 1.0f && partialTick != realPartialTick) {
            partialTick = realPartialTick;
        }
